...
static void decode_task()
{
    ...
    while (1)
    {
        ...
        if (decoded_num)
        {
            ...
            environment_variable.face_ID = (int)strtol(result.data, NULL, 10);
            // Publish the data
            gate_publish();
        }
...
static esp_err_t wifi_event_handle(void *ctx, system_event_t *event)
{
    ...
    case SYSTEM_EVENT_STA_GOT_IP:
        ...
        if (mqtt_started == false)
        {
            ...
            // xTaskCreate((void (*)(void *))gate_main, "gate_main", 10240, NULL, 5, NULL);
            // 创建一个新的任务来启动MQTT客户端
            mqtt_started = true;
            // 将mqtt_started标志设置为true，表示MQTT客户端已启动
            // xTaskNotifyGive(network_connected_task);
            // 设置任务通知，表示网络建立成功
        }
        break;
    default:
...
void uart_receive_task(void *pvParameters)
{
    ...
    while(1)
    {
        ...
        // 检查uart_read_bytes的返回值，确定是否出现错误
        if(len < 0)
        {
            ESP_LOGE("UART", "UART read bytes error");
        }
        else if(len > 0)
        {
            // 打印接收到的数据
        ...
        // 一般在任务的while(1)循环的最后，要加上一定的延时以释放CPU给其他任务
        // 可以根据你的具体任务需求，调整此延时
        vTaskDelay(1000 / portTICK_PERIOD_MS);
    }
...
void app_main()
{
    ...
    HAL_SetProductKey("a19sCsAuZcM");

    HAL_SetDeviceName("AI_model_1");
    HAL_SetDeviceSecret("e534c89f12c62b08764a42403133d168");
    ...
    // 设置Wi-Fi配置
    conn_mgr_set_wifi_config_ext((const uint8_t *)wifi_ssid, strlen(wifi_ssid), (const uint8_t *)wifi_pass, strlen(wifi_pass));
    conn_mgr_start();
